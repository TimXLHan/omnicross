...
sumoCmd = ["/app/bin/sumo-gui", "-c", "/home/simulator/simulation/sumo.cfg.xml"]

# Example of hash map
vehicle_order = {"car1": 1, "car2": 2, "car3": 3, "car4": 4}
...
while traci.simulation.getMinExpectedNumber() > 0:
    ...
    for vehicle_id in list(vehicle_order.keys()): # Note the use of list() here to make a copy
        if vehicle_id in traci.vehicle.getIDList():
            # If the vehicle has cleared the junction or all before it have cleared, let it go
            if has_cleared_junction(vehicle_id) or all_before_cleared(vehicle_id, vehicle_order):
            ...
            if has_cleared_junction(vehicle_id):
                del vehicle_order[vehicle_id]
traci.close()
